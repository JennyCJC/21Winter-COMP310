...
void initIO(){
    ...
    for (int i = 0; i < MAX_FILES; i++) {
        ...
        blankFile.readPermission = 0;
        blankFile.writePermission = 0;
    }
...
int partition(char *name, int blocksize, int totalblocks){
    ...
    //MODIFICATION: use global variable path to store current partition name
...
char *readBlock(int file){
    ...
    //MODIFICATION
    for(int i = fat[file].offset; i > 0; i--){
        fgetc(filePtr);
    }
...
void updateFATOnDisk(){
    ...
    for (int i = 0; i < MAX_FILES; i++) {
        ...
        //MODIFICATION
        sprintf(writebuffer2, "%s, %i, %d, %d\n %s\n", currentFile.filename, currentFile.file_length,currentFile.readPermission,currentFile.writePermission ,writebuffer);
