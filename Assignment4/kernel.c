...
int scheduler(){
    ...
    while (size() != 0){
        ...
        if(pcb->pageTable[pcb->PC_page] != -1){
            //if == 1 : page fault: this page has been a victim page before, codes in has gone
            errorCode = run(quanta);
        }else{
            printf("victmed page here.\n");
            errorCode = 100; //pagefault: deal with swapped victim frame
        }

        
        if (errorCode == 1 || errorCode == 100){    // indicating page fault
            if(errorCode == 100){
                errorCode = pageFault(pcb,true);
            }else{
                errorCode = pageFault(pcb,false);
            }
            if(errorCode == 1){
                free(pcb);
            }else{
